...
function GridAreaItem() {
  ...
  const onMouseMove = useCallback(
    useThrottling((event: globalThis.MouseEvent) => {
      dispatch({
  ...
  const onItemSelected: MouseEventHandler<HTMLElement> = useCallback(
    (event) => {
      mousePosRef.current = [event.clientX, event.clientY];
      setIsGrabbed(true);
    },
  ...
  return (
    <div
      className={`${styles["grid-area-item"]} ${
        isGrabbed ? styles["grid-area-item--grabbed"] : ""
      }`}
