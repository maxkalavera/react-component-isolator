...
describe('Component ZoomBar', () => {
  ...
  test('should increase Zoom when up button is pressed', async () => {
    ...
    await waitFor(() => expect(parseFloat(callback.mock.results[callback.mock.results.length - 1].value)).toBeGreaterThan(1.0))
  })

  test('should decrease Zoom when down button is pressed', async () => {
    fireEvent.click(await screen.findByTestId('zoombar-down'))
    await waitFor(() => expect(parseFloat(callback.mock.results[callback.mock.results.length - 1].value)).toBeLessThan(1.0))
  })
