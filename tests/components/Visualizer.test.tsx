...
import {fireEvent, render, screen, waitFor, InspectReactIsolatorContext, testingInitialContextState, cleanup} from "src/utils/test-utils";
...
describe('Components Visualizer', () => {
  ...
  test('Element in grid area can be dragged', async () => {
    ...
    fireEvent.mouseMove(selectedItem, {clientX: mouseX + 20, clientY: mouseY + 20})
    fireEvent.mouseMove(selectedItem, {clientX: mouseX + 40, clientY: mouseY + 40})
    await waitFor(() => expect(testContext.callback).toBeCalledTimes(callbackCountBefore))

    // Mouse the mouse with the item selected
    fireEvent.mouseMove(selectedItem, {clientX: mouseX, clientY: mouseY})
    fireEvent.mouseDown(selectedItem)
    fireEvent.mouseMove(selectedItem, {clientX: mouseX + 10, clientY: mouseY + 10})
    await waitFor(() => expect(testContext.callback).toHaveLastReturnedWith({x: 10, y: 10}))
  ...
  test('clicking show grid switch button should turn on grid option', async () => {
    ...
    // Turn off
    fireEvent.click(await screen.findByTestId('grid-switch-button'))
    await waitFor(() => expect(testContext.callback).toHaveLastReturnedWith(false))
  })
