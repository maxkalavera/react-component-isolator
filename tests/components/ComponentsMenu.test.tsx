...
describe("Component ComponentsMenu", () => {
  test("Renders correctly", async () => {
    ...
    await waitFor(() =>
      expect(callback).toHaveLastReturnedWith("Second element")
    );
    ...
    expect(await screen.findByText("First element")).toBeInTheDocument();
    await waitFor(() =>
      expect(screen.queryByText("Second element")).not.toBeInTheDocument()
    );
    await waitFor(() =>
      expect(screen.queryByText("Third element")).not.toBeInTheDocument()
    );
  });
});
