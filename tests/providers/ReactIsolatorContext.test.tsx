...
import {render, waitFor, InspectReactIsolatorContext, testingInitialContextState, cleanup, act} from "src/utils/test-utils"
...
describe('Provider ReactIsolatorContext', () => {
  const testContext: {
    ...
    initialContextState: {
      ...
      isolatedItems: ['First element', 'Second element', 'Third element'].map((item, index) => ({
        name: item,
        id: `${index}`,
  ...
  beforeEach(() => {
    render(
      <ReactIsolatorContextProvider initialState={testContext.initialContextState}>
  ...
  test('When selected an item should be centered in the canvas', async () => {
    ...
    act(() => {
      testContext.dispatch && testContext.dispatch({type: 'SET_SELECTED_ITEM_ID', payload: '1'})
    })

    // Test if the value is around the expected plus/minus 1.0
    await waitFor(() => expect(selectedItemIDCallback).toHaveBeenLastCalledWith('1'))
    let lastPosition = selectedItemPositionCallback.mock.lastCall[0]
    expect(Math.abs(centeredPosition.x - lastPosition.x)).toBeLessThanOrEqual(1.0)
    expect(Math.abs(centeredPosition.y - lastPosition.y)).toBeLessThanOrEqual(1.0)
  })

  test('Selected item could never be outside the canvas cords', async () => {
    testContext.callback = jest.fn((context: ReactIsolatorContext) => context.selectedItemPosition)
    ...
    await waitFor(() => expect(testContext.callback).toHaveLastReturnedWith(centeredPosition))
    act(() => {
      testContext.dispatch && testContext.dispatch({
        type: 'SET_SELECTED_ITEM_POSITION', 
        payload: {x: -1, y: -1}
    ...
    await waitFor(() => expect(testContext.callback).toHaveLastReturnedWith({x: 0,  y: 0}))
    act(() => {
      testContext.dispatch && testContext.dispatch({
        type: 'SET_SELECTED_ITEM_POSITION', 
        payload: {x: Math.floor(canvasSize.width + 1), y: Math.floor(canvasSize.height + 1)}
      })
    })
    await waitFor(() => expect(testContext.callback).toHaveLastReturnedWith({x: canvasSize.width, y: canvasSize.height}))
  })
})